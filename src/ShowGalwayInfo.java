...
public class ShowGalwayInfo  extends JFrame implements ListSelectionListener,ActionListener{
		private County countyGalway = null;
		...
		private String name = "images/galwayMap.png";
		private int count =0;
		...
		public ShowGalwayInfo(){
			super("Information Galway");
		...
		public JPanel buildCenterPanel(){
			countyGalway = new GalwayCounty();
			countyGalway.getInfo();
			syn = countyGalway.getSynopsis().getSynop();
		...
		public void getResults(String s){
			...
			if(s == "Hotels"){
				countyGalway= new GalwayCounty();	//create an object of type DublinCounty
				out = countyGalway.getHotels().getHotelName(); //access the methods of DublinCounty
				size=0;
				String[] list =null;
				...
				countyGalway = new GalwayCounty();
				out = countyGalway.getRestaurants().getRestaurant();
				size=0;
				String[] list =null;
				...
				countyGalway = new GalwayCounty();
				out = countyGalway.getPlacesOfInterests().getPlace();
				size=0;
				String[] list =null;
				StringTokenizer st = new StringTokenizer(out);
				size = st.countTokens();
				...
				countyGalway = new GalwayCounty();
				out = countyGalway.getThingsToDo().getThing();
				size=0;
				String[] list =null;
				StringTokenizer st = new StringTokenizer(out);
				size = st.countTokens();
		...
		public void valueChanged(ListSelectionEvent e) {
			if(e.getValueIsAdjusting()){
				//Hotel List
				if(t.getSelectedValue().equals("HarbourHotelGalway")){
					int index = t.getSelectedIndex();
					listDataHotels(index);
				}//end inner if
				else if(t.getSelectedValue().equals("JurysInnGalway")){
					int index = t.getSelectedIndex();
					listDataHotels(index);
				}
				else if(t.getSelectedValue().equals("MaldronHotelGalway")){
					int index = t.getSelectedIndex();
					listDataHotels(index);
				}
				else if(t.getSelectedValue().equals( "RadissonBluHotel&SpaGalway")){
					int index = t.getSelectedIndex();
					listDataHotels(index);
				}
				//Restaurants List
				else if(t.getSelectedValue().equals("TheQuayStreetKitchen")){
					int index = t.getSelectedIndex();
					listDataRes(index);
				}
				else if(t.getSelectedValue().equals("OranTandoori")){
					int index = t.getSelectedIndex();
					listDataRes(index);
				}
				else if(t.getSelectedValue().equals("TheChiliShack")){
					int index = t.getSelectedIndex();
					listDataRes(index);
				}
				else if(t.getSelectedValue().equals("DaPaulino'sRistoranteItaliano")){
					int index = t.getSelectedIndex();
					listDataRes(index);
				}
				//Places to Vist
				else if(t.getSelectedValue().equals("GalwayCathedral")){
					int index = t.getSelectedIndex();
					listDataPlace(index);
				}
				else if(t.getSelectedValue().equals("QuayStreet")){
					int index = t.getSelectedIndex();
					listDataPlace(index);
				}
				else if(t.getSelectedValue().equals("NationalUniversityofIreland-Galway")){
					int index = t.getSelectedIndex();
					listDataPlace(index);
				}
				else if(t.getSelectedValue().equals("TheSpanishArch")){
					int index = t.getSelectedIndex();
					listDataPlace(index);
				}
				//Things List
				else if(t.getSelectedValue().equals("WildAtlanticWayDayTours")){
					int index = t.getSelectedIndex();
					listDataThings(index);
				}
				else if(t.getSelectedValue().equals("ConnemaraPubTours")){
					int index = t.getSelectedIndex();
					listDataThings(index);
				}
				else if(t.getSelectedValue().equals("CorribPrincessRiverCruise")){
					int index = t.getSelectedIndex();
					listDataThings(index);
				}
				else if(t.getSelectedValue().equals("SegwayAdventureToursofGalway")){
					int index = t.getSelectedIndex();
					listDataThings(index);
				}
		...
		public void actionPerformed(ActionEvent e) {
			if(e.getSource() == west1){
				...
				String result = countyGalway.getNames().getName();
				la3.setText(result);
			}
			else if(e.getSource() == east2){
				String result = countyGalway.getDescriptions().getDescription();
				la3.setText("<html>" + result +  "<br></html>");
			}
			else if(e.getSource() == east3){
				String result = countyGalway.getLocations().getLocation();
				la3.setText("<html>" + result +  "<br></html>");
			}
		...
		public void listDataHotels(int index){
			countyGalway.setDetailHotel(index);
			String output = countyGalway.getNames().getName() + countyGalway.getDescriptions().getDescription() + countyGalway.getLocations().getLocation();
